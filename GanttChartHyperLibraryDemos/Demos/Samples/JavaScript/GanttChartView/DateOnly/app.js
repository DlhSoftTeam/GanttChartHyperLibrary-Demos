...
// Use the same formatter for date-time as for date.
settings.dateTimeFormatter = settings.dateFormatter;

var isDuringChangingDate = false;
// Handle item property change notifications: when times change, set them to the start/end of day, e.g. based on automatic scheduling constraints.
settings.itemPropertyChangeHandler = function (item, propertyName, isFinal) {
    if (!isDuringChangingDate && isFinal && (propertyName == 'finish' || propertyName == 'start')) {
        isDuringChangingDate = true;
        var start = new Date(item.start);
        start.setHours(start.getHours() - 8);
        ganttChartView.setItemStart(item, start);
        var finish = new Date(item.finish);
        finish.setHours(finish.getHours() + 8);
        ganttChartView.setItemFinish(item, finish);
        isDuringChangingDate = false;
    }
}
