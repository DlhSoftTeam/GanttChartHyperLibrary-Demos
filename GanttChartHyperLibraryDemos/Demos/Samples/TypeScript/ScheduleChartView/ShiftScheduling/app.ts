...
for (var i = 1; i <= engineerCount; i++) {
    var scheduleChartItem = <ScheduleChartItem>{ content: 'Engineer #' + i, start: new Date(), ganttChartItems: [] };
    scheduleChartItems.push(scheduleChartItem);
}
for (var i = 1; i <= managerCount; i++) {
    var scheduleChartItem = <ScheduleChartItem>{ content: 'Manager #' + i, start: new Date(), ganttChartItems: [] };
    scheduleChartItems.push(scheduleChartItem);
}
...
var customScale = <GanttChartView.Scale>{ scaleType: 'Custom', headerTextFormat: 'Custom', intervals: customIntervals, headerStyle: 'padding: 2.25px; border-right: solid 1px White; color: Gray', isSeparatorVisible: true, separatorStyle: 'stroke: #c8bfe7; stroke-width: 0.25px' };
settings.scales = [
...
for (var i = 0; i < engineerCount; i++) {
    ...
    for (var d = new Date(settings.timelineStart.valueOf() - 1 * hourDuration + (i % 4) * shiftDuration); d < settings.timelineFinish; d = new Date(d.valueOf()), d.setHours(d.getHours() + (shiftDuration / hourDuration) * 4)) {
        ...
        ganttChartItems.push({
            ...
            finish: new Date(d.valueOf() + shiftDuration - 15 * minuteDuration), // Preserve 30 minutes before and after shifts.
...
for (var i = 0; i < managerCount; i++) {
    ...
    for (var d = new Date(settings.timelineStart.valueOf() - 1 * hourDuration + (i % 3) * shiftDuration); d < settings.timelineFinish; d = new Date(d.valueOf()), d.setHours(d.getHours() + (shiftDuration / hourDuration) * 3)) {
        ...
        ganttChartItems.push({
            ...
            start: new Date(d.valueOf() + 15 * minuteDuration),
            finish: new Date(d.valueOf() + shiftDuration - 15 * minuteDuration), // The same as for engineers.
