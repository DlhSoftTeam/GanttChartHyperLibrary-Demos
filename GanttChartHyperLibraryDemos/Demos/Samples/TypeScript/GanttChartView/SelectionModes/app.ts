...
var selectionModeSelect = <HTMLSelectElement>document.querySelector('#selectionModeSelect');
var selectionCountOutput = <HTMLElement>document.querySelector('#selectionCountOutput');

// Set selection settings based on user selections.
function initialize() {
    // Clear selection.
    ...
    settings.itemSelectionChangeHandler = () => {
        setTimeout(() => {
            selectionCountOutput.innerText = ganttChartView.getSelectedItems().length.toString();
        });
    }
...
// Update selection mode upon user command.
selectionModeSelect.addEventListener('change', initialize);
