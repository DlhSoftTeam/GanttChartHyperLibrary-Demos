...
// Set custom fields to configure custom templates.
...
columns.splice(3, 0, {
    ...
    cellTemplate: (item: MyGanttChartItem) => {
        return DlhSoft.Controls.GanttChartView.optionSelectColumnTemplateBase(document, 124,
            () => { return ["No", "Yes"]; },
            () => { return item.hasMilestoneAtFinish ? "Yes" : "No"; },
            (value) => {
...
columns.splice(4, 0, {
    ...
    cellTemplate: (item: MyGanttChartItem) => {
        return DlhSoft.Controls.GanttChartView.numberInputColumnTemplateBase(document, 124,
            () => { return item.numberOfLinesToDisplayInsteadOfRectangle; },
            (value) => {
...
function applyStyle(line: SVGElement, item: GanttChartItem, settings: GanttChartView.Settings): void {
...
function linesTemplate(item): SVGElement {
...
function getFinishDiamond(item): SVGElement {
    var ganttChartView = item.ganttChartView;
    var settings = ganttChartView.settings;
    var document = ganttChartView.ownerDocument;
    var undefinedType = 'undefined', svgns = 'http://www.w3.org/2000/svg';
